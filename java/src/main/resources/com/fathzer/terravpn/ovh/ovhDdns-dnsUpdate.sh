...
# Store arguments with proper quoting
USER="$1"
PASSWORD="$2"
HOSTNAME="$3"
IP="$4"
...
echo "[DynHost] Exécution de la commande wget..."
...
if [ $WGET_EXIT -ne 0 ]; then
    echo "[DynHost] Erreur lors de l'exécution de wget (code $WGET_EXIT)"
    echo "[DynHost] Sortie de wget:"
    echo "$RESPONSE"
    exit 1
fi
...
if echo "$RESPONSE" | grep -q -E '^nohost|^notfqdn|^badagent|^badauth|^badresolv|^badparam|^error'; then
    echo "[DynHost] Erreur lors de la mise à jour DNS"
    exit 1
fi

echo "[DynHost] Mise à jour DNS réussie"
exit 0
